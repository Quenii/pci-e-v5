...
* After integrating the design with user's testbench, the user needs to add
  these files (testbench files, toplevel wrapper file) in ddr2_sdram_mig33.prj under
  synth folder.
...
* "ddr2_sdram_mig33.ucf" file is the constraint file for the design. This is used by
...
* "mem_interface_top.ut" file has the options for the Configuration file
  generation i.e. the "ddr2_sdram_mig33.bit" file to run in batch mode.
...
* "icon4_cg.xco", "vio_async_in96_cg.xco", "vio_async_in100_cg.xco",
  ...
  ddr2_sdram_mig33 rtl file. In order to generate the EDIF/NGC files, you must
...
Note : When you generate the design using DEBUG_EN option, the above mentioned
       ...
       create_ise.bat files. The ddr2_sdram_mig33 rtl file will have the design
...
* At the start of a Chip Scope Analyzer project, all of the signals in
  every core have generic names. "ddr2_sdram_mig33.cdc" is a file that contains
  all the signal names of all cores. Upon importing this file, signal names are
  renamed to the specified names in "ddr2_sdram_mig33.cdc" file. This file will work
...
* Synth folder has the constraint file for Synplify Pro designs
  i.e. the "mem_interface_top_synp.sdc" file, "ddr2_sdram_mig33.prj" file which
  has the design files to be added to the project, "script_synp.tcl" file
  that sets the synthesis options and "ddr2_sdram_mig33.lso" file which is a custom
...
* For example, the design is generated with component name as ddr2_sdram_mig33 with
  Target FPGA LX50t-FF1136 and Compatible FPGA LX85t-FF1136.
   ...
   - Change the UCF file name in batch file to the one in compatible_ucf
     folder i.e., change the UCF name (ddr2_sdram_mig33.ucf) in ise_flow.bat and
