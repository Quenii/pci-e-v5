...
//   Prints the pipes information for all active pipes in the device
// Parameters:
...
//   Find a pipe in any of the device's active interfaces
// Parameters:
//   pDevice [in] pointer to the USB device
//   dwPipeNumber [in] the pipe number to look for
// Return Value:
//   a pointer to the pipe or NULL if no such pipe exists
static WDU_PIPE_INFO *FindPipeInDevice(WDU_DEVICE *pDevice, DWORD dwPipeNumber)
{
    ...
    for (i=0; i<pDevice->pActiveConfig->dwNumInterfaces; i++)
    {
        ...
        WDU_ALTERNATE_SETTING *pAltSet = pIfc->pActiveAltSetting;
...
End_transfer:
        ...
        case MENU_RW_LISTEN_PIPE:
            ...
            {
                listenPipe.dwBytesToTransfer = pPipe->dwMaximumPacketSize * 8; // suit 8 minimum packets for high speed transfers 
